...
本文查看tapable中 AsyncParallelHook 中订阅和发布的逻辑

异步的hook中订阅有三种方式tap,tapAsync,tapPromise,调用有2中方式callAsync,promise

### 1. tap订阅 callAsync发布和 promise发布
...
```
对比两个函数可以看到callAsync和promise两种发布方式在函数处理上类似，在处理结果的返回上有差异，callAsync是以callback方式返回，promise是以promise方式返回

tap订阅的订阅方法是以 _fn0(arg1, arg2) 直接调用的方式
...
### 2. tapAsync订阅 callAsync发布和 promise发布
...
```
### 3. tapPromise订阅 callAsync发布和 promise发布
